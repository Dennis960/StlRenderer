...
const DEFAULT_COLORS = ['#cccccc', '#c88a8a', '#8ac88a', '#c8c88a', '#8ac8c8', '#c88ac8'];
...
$('resetBtn').addEventListener('click', () => {
  ...
  if (currentGroup) {
    ...
    // Reset each mesh's color to its default palette entry
...
/** Render the per-model color pickers into #modelColorsList. */
...
function updateCurl() {
  ...
  loadedMeshes.forEach(m => {
    p.append('color', m.material.color.getHexString());
  });
...
async function doRender() {
  ...
  loadedMeshes.forEach(m => {
    params.append('color', m.material.color.getHexString());
  });
